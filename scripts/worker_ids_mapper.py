...
data. The original worker ids are stored in restricted access file that is
removed from the public repository ("data/mturk_worker_ids_mapping.csv").
"""
...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk("results/user_study_output/"):
        for file in files:
    ...
    unique_worker_ids_mapping_pd.to_csv(
        "results/user_study_output/mturk_worker_ids_mapping.csv", index=False
    )
